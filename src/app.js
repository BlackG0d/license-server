...
async function loadAndValidateLicenseForStart(licenseKey, email, deviceId) {
    ...
    // If already active, ensure same email/device
...
async function ensureLicenseStillValid(licenseId) {
    const license = await get("SELECT * FROM licenses WHERE id = ?", [licenseId]);
    if (!license) {
...
/**
 * Step 1: Start registration (needs licenseKey + email + deviceId)
 */
app.post("/auth/start-registration", async (req, res) => {
    try {
        ...
        // If unused -> activate + bind to email/device
      ...
      `,
            ...
            await run(`UPDATE licenses SET updated_at = ? WHERE id = ?`, [nowIso, license.id]);
        }

        // Optional: block if user already exists with this email
        ...
        const now = new Date();
        const expiresAt = addMinutes(now, 10); // 10 minutes
    ...
    `,
        ...
        // TODO: send real email – currently console.log
...
/**
 * Step 2: Verify email code, issue password setup token
 */
app.post("/auth/verify-email", async (req, res) => {
    try {
      ...
      WHERE email = ? AND device_id = ? AND used = 0
...
app.post("/auth/set-password", async (req, res) => {
    try {
        ...
        await run(
            `
      INSERT INTO users (email, password_hash, device_id, license_id, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `,
            [email, passwordHash, deviceId, licenseId, nowIso, nowIso]
        );
...
/**
 * Step 4: Login (one device per account + license active)
 */
app.post("/auth/login", async (req, res) => {
    try {
        ...
        // If device not yet bound, bind it now
        if (!user.device_id) {
            await run("UPDATE users SET device_id = ?, updated_at = ? WHERE id = ?", [
                deviceId,
...
app.get("/me", authMiddleware, async (req, res) => {
    try {
        ...
        const user = await get(
            "SELECT id, email, device_id, license_id, created_at FROM users WHERE id = ?",
            [userId]
        );
        if (!user) {
        ...
        const license = await get(
            "SELECT license_key, status, used_at FROM licenses WHERE id = ?",
            [user.license_id]
        );
