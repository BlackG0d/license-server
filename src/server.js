...
/**
 * Health check (optional, useful for Vercel)
 */
app.get("/health", async (req, res) => {
    try {
        const row = await get("SELECT 1 as ok", []);
        return res.json({ status: "ok", db: row ? row.ok : null });
    } catch (e) {
...
module.exports = app; // so Vercel can import it via api/index.js if you use that
